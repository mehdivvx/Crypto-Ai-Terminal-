...
export default function ChartPanel() {
  ...
  // FIX: Now ONLY finds the signal if it is actively taken ('ACTIVE'), ignoring 'PENDING'
  const currentSignal = signals.find(s => s.symbol === activePair && s.status === 'ACTIVE');

  // --- TRADINGVIEW ENGINE (High Performance Refs, No React Lag) ---
  ...
  const draw = useCallback(() => {
    ...
    // ─── UPGRADED VOLUME RENDERING ───
    const mxV = Math.max(...visibleCandles.map(c => c.v), 1);
    const maxVolHeight = chartH * 0.25; // Volume bars now max out at 25% of the chart height (much taller)
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      // Increased opacity from 0.15 to 0.35 for much better visibility
      ctx.fillStyle = c.c >= c.o ? 'rgba(0,255,157,0.35)' : 'rgba(255,42,109,0.35)';
      ctx.fillRect(x - cW * 0.4, H - pad.b - vH, cW * 0.8, vH);
    }

    // DRAW REAL SIGNAL ZONES (Only draws if currentSignal exists, meaning trade is ACTIVE)
    ...
    if (p && p >= paddedMnL && p <= paddedMxH) {
      const py = sy(p);
      ctx.strokeStyle = 'rgba(0,212,255,0.8)'; ctx.lineWidth = 1; ctx.setLineDash([3, 3]);
      ctx.beginPath(); ctx.moveTo(pad.l, py); ctx.lineTo(W - pad.r, py); ctx.stroke();
      ctx.setLineDash([]);
      
      ctx.fillStyle = 'rgba(0,212,255,0.15)'; ctx.fillRect(W - pad.r + 2, py - 10, pad.r - 4, 20);
      ctx.fillStyle = '#00d4ff'; ctx.font = 'bold 11px "JetBrains Mono"'; ctx.textAlign = 'left';
      ctx.fillText(p.toFixed(activePair?.includes("BTC") ? 1 : 4), W - pad.r + 8, py + 4);
    }
  }, [candles, activePair, isFullscreen, currentSignal]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => requestAnimationFrame(draw), 100);
    return () => {
      ro.disconnect();
