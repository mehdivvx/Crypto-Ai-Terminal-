...
export default function Heatmap() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#11141a] border border-white/10 p-3 rounded-lg shadow-xl font-mono text-[11px] z-50">
          <p className="text-white mb-2 font-bold border-b border-white/10 pb-1">Price: {formatPrice(label, activePair)}</p>
          {payload.map((entry: any, index: number) => {
            if(entry.name.includes("Cumulative")) return null; 
            return (
              <div key={index} className="flex justify-between gap-4 py-0.5">
                <span style={{ color: entry.color }}>{entry.name}:</span>
                <span className="text-white font-bold">{formatLargeNumber(entry.value)} {activePair.split('/')[0]}</span>
              </div>
            )
          })}
        </div>
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#0b0e14] rounded-[16px] border border-white/5 overflow-hidden">
      
      <div className="flex flex-col border-b border-white/5 shrink-0">
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="bg-[#1c2128] text-[#00f0ff] px-3 py-1.5 rounded-md flex items-center gap-2 font-mono text-[12px] font-bold border border-[#00f0ff]/20 shadow-[0_0_10px_rgba(0,240,255,0.1)]">
              <Activity size={14} />
              Macro Depth Map
            </div>
            <h2 className="text-white font-display text-[16px] md:text-[18px] font-bold tracking-wide ml-2">
              Binance {activePair} Heatmap
            </h2>
          </div>

          <div className="flex items-center gap-2 md:gap-3">
            <button className="bg-[#1c2128] border border-white/5 text-[#a0a8b3] p-1.5 rounded-md hover:text-white hover:bg-white/5 transition-colors">
              <RefreshCw size={14} className={isFetching ? "animate-spin" : ""} />
            </button>
          </div>
        </div>

        <div className="flex justify-center items-center gap-16 px-6 py-4 bg-[#0a0c10] border-t border-white/5">
           <div className="flex flex-col items-center">
              <span className="text-[10px] font-mono text-[#8b99ae] tracking-wider mb-1">
                MACRO BIDS <span className="opacity-60">(Support Walls)</span>
              </span>
              <span className="text-[20px] font-num font-bold text-[#00ff9d]" style={{ textShadow: '0 0 15px rgba(0,255,157,0.3)' }}>
                {formatUsdt(totals.bidUsdt)}
              </span>
           </div>
           
           <div className="w-px h-[35px] bg-white/10 mx-2"></div>
           
           <div className="flex flex-col items-center">
              <span className="text-[10px] font-mono text-[#8b99ae] tracking-wider mb-1">
                MACRO ASKS <span className="opacity-60">(Resistance Walls)</span>
              </span>
              <span className="text-[20px] font-num font-bold text-[#ff2a6d]" style={{ textShadow: '0 0 15px rgba(255,42,109,0.3)' }}>
      ...
      <div className="flex-1 w-full min-h-0 relative p-4 pl-0">
        {depthData.length === 0 ? (
           <div className="w-full h-full flex items-center justify-center font-mono text-[12px] text-[#4f5b70] animate-pulse">
            ...
            <ComposedChart data={depthData} margin={{ top: 45, right: 20, left: 20, bottom: 0 }}>
              <defs>
                <linearGradient id="colorBid" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={COLORS.cumBid} stopOpacity={0.3} />
                  <stop offset="95%" stopColor={COLORS.cumBid} stopOpacity={0} />
                </linearGradient>
                <linearGradient id="colorAsk" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={COLORS.cumAsk} stopOpacity={0.3} />
                  <stop offset="95%" stopColor={COLORS.cumAsk} stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <YAxis 
                yAxisId="right" orientation="right" stroke="#545f73"
                tick={{ fill: '#545f73', fontSize: 10, fontFamily: 'monospace' }}
                tickLine={false} axisLine={false}
                tickFormatter={(value) => formatLargeNumber(value)}
                dx={10}
              ...
              {/* Tooltip kept, Legend completely removed */}
              <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.05)' }} />

              {/* Names kept here so Tooltip still shows proper labels, but Legend is gone */}
              <Bar dataKey="bidVol" name="Bid Wall (Buyers)" fill={COLORS.bidWall} yAxisId="right" maxBarSize={6} />
              <Bar dataKey="askVol" name="Ask Wall (Sellers)" fill={COLORS.askWall} yAxisId="right" maxBarSize={6} />

              <Area type="monotone" dataKey="cumBid" name="Cumulative Bids" stroke={COLORS.cumBid} strokeWidth={2} fill="url(#colorBid)" yAxisId="left" dot={false} activeDot={false} connectNulls />
              <Area type="monotone" dataKey="cumAsk" name="Cumulative Asks" stroke={COLORS.cumAsk} strokeWidth={2} fill="url(#colorAsk)" yAxisId="left" dot={false} activeDot={false} connectNulls />
                ...
                label={({ viewBox }: any) => {
                  ...
                  return (
                    <g transform={`translate(${viewBox.x}, ${viewBox.y})`}>
                      {/* Price Text */}
                      <text x={0} y={-18} fill="#ffffff" fontSize={14} fontFamily="monospace" fontWeight="bold" textAnchor="middle" style={{ textShadow: '0 0 10px rgba(0,0,0,0.8)' }}>
                        {formatPrice(currentPrice, activePair)}
                      </text>
                      {/* Arrow Pointing Down */}
                      <text x={0} y={-4} fill="#ffffff" fontSize={12} textAnchor="middle" style={{ textShadow: '0 0 10px rgba(0,0,0,0.8)' }}>
                        ▼
                      </text>
                    </g>
                  );
